...
export async function registerRoutes(
  ...
  app.get("/api/daily-challenge", requireAuth as any, async (req: Request, res: Response) => {
    ...
    // Full string hash (not just one character) so questions don't cluster on
    // colliding seeds once the bank is large (dc1 vs dc10 vs dc100 used to hash
    // the same via charCodeAt(2) — that's part of why SPI/IDIQ kept resurfacing).
    ...
    // Question bank lives in ./dailyChallengeQuestions.ts — pulled from every module's
    // lesson quizzes and module assessments in client/src/lib/curriculum.ts (430 questions,
    // up from the 20 hardcoded here before). See that file's header for how to regenerate it.
    const questionBank = dailyChallengeQuestionBank;

    // Seed-shuffle and pick 5
    const shuffled = [...questionBank].sort((a, b) => {
      const ra = seededRandom(seed + hashId(a.id));
      const rb = seededRandom(seed + hashId(b.id));
      return ra - rb;
    });
